...
function Gauss_apply(gauss_numb::Int, threads_apply_gauss::Tuple{Int64,Int64,Int64}, blocks_apply_gauss::Tuple{Int64,Int64,Int64}, currGaussIndex::Int)::Gauss_apply_str
...
function Lux.initialparameters(rng::AbstractRNG, l::Gauss_apply_str)
    return ((stdGaus = CUDA.ones(l.gauss_numb)), means=CuArray(Float32.((collect(0:(l.gauss_numb-1))) ./ (l.gauss_numb - 1))))
end
function Lux.initialstates(::AbstractRNG, l::Gauss_apply_str)::NamedTuple
    return (meansLength=l.gauss_numb, threads_apply_gauss=l.threads_apply_gauss, blocks_apply_gauss=l.blocks_apply_gauss, currGaussIndex=l.currGaussIndex)
...
@inline function thresholdIter(arr, mean, std, f1Channel, f2channel, x, y, z, xChange, yChange, zChange)
    return multThreshold(arr[x, y, z, 1, 1], arr[x+xChange, y+yChange, z+zChange, 1, 1], mean, std, arr[x, y, z, f1Channel, 1], arr[x+xChange, y+yChange, z+zChange, f1Channel, 1], arr[x, y, z, f2channel, 1], arr[x+xChange, y+yChange, z+zChange, f2channel, 1])
end
...
"""
function applyGaussKern_Deff(means, d_means, stdGaus, d_stdGaus, origArr, d_origArr, out, d_out, meansLength, currGaussIndex)

    Enzyme.autodiff_deferred(applyGaussKern, Const, Duplicated(means, d_means), Duplicated(stdGaus, d_stdGaus), Duplicated(origArr, d_origArr), Duplicated(out, d_out), Const(meansLength), Const(currGaussIndex)
    )
    return nothing
end
...
function (l::Gauss_apply_str)(origArr, ps, st::NamedTuple)
    ...
    out1 = callGaussApplyKern(ps.means, ps.stdGaus, origArr, st.meansLength, st.threads_apply_gauss, st.blocks_apply_gauss, 1)

    out2 = callGaussApplyKern(ps.means, ps.stdGaus, origArr, st.meansLength, st.threads_apply_gauss, st.blocks_apply_gauss, 2)
    out3 = callGaussApplyKern(ps.means, ps.stdGaus, origArr, st.meansLength, st.threads_apply_gauss, st.blocks_apply_gauss, 3)
    out4 = callGaussApplyKern(ps.means, ps.stdGaus, origArr, st.meansLength, st.threads_apply_gauss, st.blocks_apply_gauss, 4)
    out5 = callGaussApplyKern(ps.means, ps.stdGaus, origArr, st.meansLength, st.threads_apply_gauss, st.blocks_apply_gauss, 5)
    out6 = callGaussApplyKern(ps.means, ps.stdGaus, origArr, st.meansLength, st.threads_apply_gauss, st.blocks_apply_gauss, 6)
    out7 = callGaussApplyKern(ps.means, ps.stdGaus, origArr, st.meansLength, st.threads_apply_gauss, st.blocks_apply_gauss, 7)
    out8 = callGaussApplyKern(ps.means, ps.stdGaus, origArr, st.meansLength, st.threads_apply_gauss, st.blocks_apply_gauss, 8)
