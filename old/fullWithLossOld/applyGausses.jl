...
function Gauss_apply(gauss_numb::Int, threads_apply_gauss::Tuple{Int64,Int64,Int64}, blocks_apply_gauss::Tuple{Int64,Int64,Int64})::Gauss_apply_str
...
function Lux.initialstates(::AbstractRNG, l::Gauss_apply_str)::NamedTuple
    return (meansLength=l.gauss_numb, threads_apply_gauss=l.threads_apply_gauss, blocks_apply_gauss=l.blocks_apply_gauss,)
...
"""
function applyGaussKern_Deff(means, d_means, stdGaus, d_stdGaus, origArr, d_origArr, out, d_out, meansLength)

    Enzyme.autodiff_deferred(applyGaussKern, Const, Duplicated(means, d_means), Duplicated(stdGaus, d_stdGaus), Duplicated(origArr, d_origArr), Duplicated(out, d_out), Const(meansLength))
    return nothing
end
...
function (l::Gauss_apply_str)(origArr, ps, st::NamedTuple)
    return callGaussApplyKern(ps.means, ps.stdGaus, origArr, st.meansLength, st.threads_apply_gauss, st.blocks_apply_gauss), st
end
l = Gauss_apply(gauss_numb_top, threads_apply_gauss, blocks_apply_gauss)
#l = Gauss_apply_str(gauss_numb_top,threads_apply_gauss,blocks_apply_gauss)
