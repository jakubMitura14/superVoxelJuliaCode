...
function prepare_for_apply_weights_to_locs_kern(control_points_shape, weights_shape)
    bytes_per_thread = 0
    blocks_apply_w, threads_apply_w, maxBlocks = computeBlocksFromOccupancy(apply_weights_to_locs_kern, (CUDA.zeros(control_points_shape...), CUDA.zeros(control_points_shape...), CUDA.zeros(weights_shape...), Float32(3), UInt32(control_points_shape[1]), UInt32(control_points_shape[2]), UInt32(control_points_shape[3])
        ), bytes_per_thread)
