...
"""
macro get_interpolated_val_in(source_arr,a,b,c)

  return  esc(quote    

      var3=(@get_dist($a,$b,$c))
      var1=var1+var3
      var2=var2+($source_arr[Int(round(shared_arr[threadIdx().x,1]+($a))),Int(round(shared_arr[threadIdx().x,2]+($b))),Int(round(shared_arr[threadIdx().x,3]+($c)))]*(var3 ))
...
macro threeDLinInterpol(source_arr)
  ...
  tmp = []
  push!(tmp, esc(:( var1=0.0 ; )))
  push!(tmp, esc(:( var2=0.0 ; )))

  # for a1 in [-0.50001,0.0,0.50001]#,0.0,0.50001
  for a1 in [-0.50001,0.0,0.50001]
    for b1 in [-0.50001,0.0,0.50001]
      for c1 in [-0.50001,0.0,0.50001]
        push!(tmp, esc(:( @get_interpolated_val_in($source_arr,$a1,$b1,$c1);)))
      end
    end
  end      
  push!(tmp, esc(:( var2=var2/var1; )))
...
#   # @three_d_loop(source_arr,:get_interpolated_val)
#   @get_interpolated_val(source_arr)
...
"""
macro get_interpolated_diff_in(source_arr,a,b,c)
  return  esc(quote       
...
macro threeD_loc_var(source_arr)
  ...
  tmp = []
  push!(tmp, esc(:( @threeDLinInterpol(source_arr); )))
  
  for a1 in [-0.50001,0.0,0.50001]
    for b1 in [-0.50001,0.0,0.50001]
      for c1 in [-0.50001,0.0,0.50001]
        push!(tmp, esc(:( @get_interpolated_diff_in($source_arr,$a1,$b1,$c1))))
      end
    end
  end      
  push!(tmp, esc(:( var2=var3/var1; )))
...
#   # @get_interpolated_diff(source_arr)#todo unhash
...
function is created to find ith sample point when max is is the number of main sample points (num_base_samp_points) +3 (for additional ones)
  ...
  num_additional_samp_points - how many additional sample points we want to have between the last main sample point and the verticies of the triangle that we used for it
    ...
    kernel is divided into two parts one is set_tetr_dat_kernel that is used to set the coordinates of the sv center and the triangle corners and the interpolated value of the point
    the second part is point_info_kern that is used to calculate the weight of the sample points and its interpolated value

    In order to make the kernel compile with Enzyme it was required to fully unroll it 
"""
# function point_info_kern(shared_arr::CuDeviceMatrix{Float32, 3},tetr_dat,out_sampled_points,source_arr,control_points,sv_centers,num_base_samp_points,num_additional_samp_points)
function set_tetr_dat_kern(tetr_dat,tetr_dat_out,source_arr,control_points,sv_centers,num_base_samp_points,num_additional_samp_points)
  ...
  # for triangle_corner_num in 2:4
  triangle_corner_num=UInt8(2)
  
  for axis in 1:3
      shared_arr[threadIdx().x,axis]=control_points[ 
        Int(tetr_dat[index,triangle_corner_num,1])
      ,Int(tetr_dat[index,triangle_corner_num,2])
      ,Int(tetr_dat[index,triangle_corner_num,3])
      ,Int(tetr_dat[index,triangle_corner_num,4])
      ,axis]
  ...
  tetr_dat_out[index,triangle_corner_num,4]=var2#todo 




  # for triangle_corner_num in 2:4
  triangle_corner_num=UInt8(3)
  ...
  @loopinfo unroll for triangle_corner_num in 2:3#we do not not need to loop over last triangle as it is already in the shared memory
  ...
  for axis in 1:3
      ...
      tetr_dat_out[index,5,axis]=shared_arr[threadIdx().x,axis]#todo 
  end#for axis
   # ########## interpolation
   var1=Float32(0.0)
   var2=Float32(0.0)
...
"""
we artificially set num_additional_samp_points to 2 and num_base_samp_points to 3
"""
